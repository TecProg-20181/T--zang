...
def set_duedate(taskID, chat):
	...
	if checkTaskId(taskID, chat):
		...
		if duedate == '':
			task.duedate = None
			db.session.commit()
			send_message("duedate deleted", chat)
			return

		task.duedate = datetime.strptime(duedate, '%Y-%m-%d')
		db.session.commit()
		send_message("Task {} duedate seted to {}".format(taskIDint, task.duedate), chat)
...
def listTasks(chat):
	...
	for task in query.all():
		...
		if task.duedate != '' and task.duedate != None:
			botStatusMessage += ' | duedate: {}'.format(task.duedate)
		botStatusMessage += '\n'
...
def startBotFunctions(command, taskID, chat):
	...
	HELP = """
		 ...
		 /duedate <task id> <date "year-month-day">
		 /duplicate <task id>
